/*
 * Copyright 2013 http4s.org
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

package io.renderback.routes

import cats.ApplicativeThrow
import cats.MonadError
import cats.MonadThrow
import cats.Semigroup
import cats.data.OptionT
import cats.effect.SyncIO
import cats.syntax.all._
import fs2.Stream
import fs2.io.file.Files
import fs2.io.file.Path
import org.http4s.Status.NotModified
import org.http4s._
import org.http4s.headers._
import org.http4s.implicits._
import org.log4s.getLogger
import org.typelevel.vault._

object StaticFile {
  private[this] val logger = getLogger

  val DefaultBufferSize = 10240

  def calculateETag[F[_]: Files: ApplicativeThrow]: Path => F[String] =
    f =>
      Files[F]
        .getBasicFileAttributes(f)
        .map(attr =>
          if (attr.isRegularFile)
            s"${attr.lastModifiedTime.toMillis.toHexString}-${attr.size.toHexString}"
          else ""
        )

  def fromPath[F[_]: Files: MonadThrow](
    f: Path,
    req: Option[Request[F]] = None,
  ): OptionT[F, Response[F]] =
    fromPath(f, DefaultBufferSize, req, calculateETag[F])

  def fromPath[F[_]: Files: MonadThrow](
    f: Path,
    req: Option[Request[F]],
    etagCalculator: Path => F[String],
  ): OptionT[F, Response[F]] =
    fromPath(f, DefaultBufferSize, req, etagCalculator)

  def fromPath[F[_]: Files: MonadThrow](
    f: Path,
    buffsize: Int,
    req: Option[Request[F]],
    etagCalculator: Path => F[String],
  ): OptionT[F, Response[F]] =
    OptionT
      .liftF(Files[F].getBasicFileAttributes(f).map(_.size))
      .flatMap { size =>
        fromPath(f, 0, size, buffsize, req, etagCalculator)
      }
      .recoverWith { case _: fs2.io.file.NoSuchFileException =>
        OptionT.none
      }

  def fromPathCompressed[F[_]: Files: MonadThrow](
    f: Path,
    req: Option[Request[F]],
  ): OptionT[F, Response[F]] = fromPathCompressed(f, DefaultBufferSize, req, calculateETag[F])

  def fromPathCompressed[F[_]: Files: MonadThrow](
    f: Path,
    buffsize: Int,
    req: Option[Request[F]],
    etagCalculator: Path => F[String],
  ): OptionT[F, Response[F]] =
    OptionT
      .liftF(findCompressed(f, req)).flatMap { case (contentFile, encoding) =>
        OptionT
          .liftF(Files[F].getBasicFileAttributes(contentFile).map(_.size))
          .flatMap { size =>
            fromPath(f, contentFile, encoding, 0, size, buffsize, req, etagCalculator)
          }
      }
      .recoverWith { case _: fs2.io.file.NoSuchFileException =>
        OptionT.none
      }

  def fromPath[F[_]: Files](
    f: Path,
    start: Long,
    end: Long,
    buffsize: Int,
    req: Option[Request[F]],
    etagCalculator: Path => F[String],
  )(implicit
    F: MonadError[F, Throwable]
  ): OptionT[F, Response[F]] = fromPath(f, f, none, start, end, buffsize, req, etagCalculator)

  private def fromPath[F[_]: Files](
    f: Path,
    contentPath: Path,
    contentEncoding: Option[ContentCoding],
    start: Long,
    end: Long,
    buffsize: Int,
    req: Option[Request[F]],
    etagCalculator: Path => F[String],
  )(implicit
    F: MonadError[F, Throwable]
  ): OptionT[F, Response[F]] =
    OptionT(for {
      etagCalc <- etagCalculator(f).map(et => ETag(et))
      _         = logger.trace(s"fromPath: $f $contentPath $contentEncoding $start..$end")
      res      <- Files[F].isRegularFile(contentPath).flatMap[Option[Response[F]]] { isFile =>
                    if (isFile) {
                      if (start >= 0 && end >= start && buffsize > 0) {
                        Files[F]
                          .getBasicFileAttributes(f)
                          .flatMap { attr =>
                            val lastModified =
                              HttpDate.fromEpochSecond(attr.lastModifiedTime.toSeconds).toOption

                            F.pure(notModified(req, etagCalc, lastModified).orElse {
                              val (body, contentLength) =
                                if (attr.size < end) (Stream.empty, 0L)
                                else (fileToBody[F](contentPath, start, end), end - start)

                              val contentType = nameToContentType(f.fileName.toString)
                              val hs          =
                                Headers(
                                  lastModified.map(`Last-Modified`(_)),
                                  `Content-Length`.fromLong(contentLength).toOption,
                                  contentType,
                                  etagCalc,
                                )

                              val r = contentEncoding.foldLeft(
                                Response(
                                  headers = hs,
                                  body = body,
                                  attributes = Vault.empty.insert(staticPathKey, f),
                                )
                              ) { (response, encoding) =>
                                response.putHeaders(
                                  `Content-Encoding`(encoding)
                                )
                              }

                              logger.trace(s"Static file generated response: $r")
                              r.some
                            })
                          }
                      } else {
                        F.raiseError[Option[Response[F]]](
                          new IllegalArgumentException(
                            s"requirement failed: start: $start, end: $end, buffsize: $buffsize"
                          )
                        )
                      }

                    } else {
                      F.pure(none[Response[F]])
                    }

                  }
    } yield res)

  private def findCompressed[F[_]: Files](
    f: Path,
    req: Option[Request[F]],
  )(implicit
    F: MonadError[F, Throwable]
  ): F[(Path, Option[ContentCoding])] = for {
    acceptEncoding  <- F.pure(req.flatMap(_.headers.get[`Accept-Encoding`]))
    range            = req.flatMap(_.headers.get[Range]).isDefined
    acceptBr         = !range && acceptEncoding.exists(_.values.toList.contains(ContentCoding.br))
    acceptGzip       = !range && acceptEncoding.exists(_.values.toList.contains(ContentCoding.gzip))
    brPath           = f.resolveSibling(f.fileName.toString + ".br")
    gzipPath         = f.resolveSibling(f.fileName.toString + ".gz")
    shouldServeBr   <- if (acceptBr) Files[F].exists(brPath) else false.pure[F]
    shouldServeGzip <- if (!shouldServeBr && acceptGzip) Files[F].exists(gzipPath) else false.pure[F]
  } yield {
    if (shouldServeBr) {
      brPath -> ContentCoding.br.some
    } else {
      if (shouldServeGzip) {
        gzipPath -> ContentCoding.gzip.some
      } else {
        f -> Option.empty
      }
    }
  }

  def notModified[F[_]](
    req: Option[Request[F]],
    etagCalc: ETag,
    lastModified: Option[HttpDate],
  ): Option[Response[F]] = {
    implicit val conjunction = new Semigroup[Boolean] {
      def combine(x: Boolean, y: Boolean): Boolean = x && y
    }

    List(etagMatch(req, etagCalc), notModifiedSince(req, lastModified)).combineAll
      .filter(identity)
      .map(_ => Response[F](NotModified))
  }

  private def etagMatch[F[_]](req: Option[Request[F]], etagCalc: ETag) =
    for {
      r          <- req
      etagHeader <- r.headers.get[`If-None-Match`]
      etagMatch   = etagHeader.tags.exists(_.exists(_ == etagCalc.tag))
      _           = logger.trace(
                      s"Matches `If-None-Match`: $etagMatch Previous ETag: ${etagHeader.value}, New ETag: $etagCalc"
                    )
    } yield etagMatch

  private def notModifiedSince[F[_]](req: Option[Request[F]], lastModified: Option[HttpDate]) =
    for {
      r          <- req
      h          <- r.headers.get[`If-Modified-Since`]
      lm         <- lastModified
      notModified = h.date >= lm
      _           = logger.trace(
                      s"Matches `If-Modified-Since`: $notModified. Request age: ${h.date}, Modified: $lm"
                    )
    } yield notModified

  private def fileToBody[F[_]: Files](f: Path, start: Long, end: Long): EntityBody[F] =
    Files[F].readRange(f, DefaultBufferSize, start, end)

  private def nameToContentType(name: String): Option[`Content-Type`] =
    name.lastIndexOf('.') match {
      case -1 => None
      case i  => MediaType.forExtension(name.substring(i + 1)).map(`Content-Type`(_))
    }

  private val staticPathKey = Key.newKey[SyncIO, Path].unsafeRunSync()

}
